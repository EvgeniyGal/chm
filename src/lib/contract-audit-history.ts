...
export type ContractAuditHistoryItem = {
  id: string;
...
const FIELD_LABELS: Record<string, string> = {
  number: "Номер",
  date: "Дата",
  signingLocation: "Місце складання",
  workType: "Тип",
  customerCompanyId: "Замовник (ID компанії)",
  contractorCompanyId: "Виконавець (ID компанії)",
  projectTimeline: "Терміни виконання",
  contractDuration: "Термін дії договору",
  signerFullNameNom: "ПІБ підписанта (називний)",
  signerFullNameGen: "ПІБ підписанта (родовий)",
  signerPositionNom: "Посада (називний)",
  signerPositionGen: "Посада (родовий)",
  signerActingUnder: "Діє на підставі",
  isSigned: "Підписаний",
  isArchived: "В архіві",
...
function displayValue(key: string, val: unknown): string {
  ...
  if ((key === "date" || key.endsWith("At")) && (typeof val === "string" || val instanceof Date)) {
...
function diffContractRows(before: Record<string, unknown>, after: Record<string, unknown>): string[] {
  const lines: string[] = [];
...
/** Human-readable lines for CONTRACT audit `diff` JSON (stored by writeAuditEvent). */
export function summarizeContractAuditDiff(action: string, diff: unknown): string[] {
  const lines: string[] = [];
  ...
  if (action === "UPDATE") {
    ...
    const after = parsed.after as Record<string, unknown> | undefined;
    if (before && after) {
      const fieldLines = diffContractRows(before, after);
      lines.push(...fieldLines);
    }

    const itemsWrap = parsed.items as { after?: unknown[] } | undefined;
    if (itemsWrap?.after && Array.isArray(itemsWrap.after)) {
      lines.push(`Оновлено перелік робіт/послуг: ${itemsWrap.after.length} поз.`);
    }
...
export async function loadContractAuditHistory(contractId: string): Promise<ContractAuditHistoryItem[]> {
  const rows = await db
    .select()
    .from(auditEvents)
    .where(and(eq(auditEvents.entityType, "CONTRACT"), eq(auditEvents.entityId, contractId)))
  ...
  return rows.map((e) => {
    ...
    const summaryLines = summarizeContractAuditDiff(e.action, parsedDiff);
    return {
      id: e.id,
      action: e.action,
...
export function auditActionLabelUa(action: string): string {
