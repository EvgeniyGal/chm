...
export async function GET(req: Request) {
  ...
  try {
    const { buffer } = await loadActiveTemplateBuffer("certificate");
    ...
    const filename = `certificate-${certNum}.docx`;
      ...
      return Response.json(
        {
          ...
          hint: "Завантажте та активуйте шаблон посвідчення в Атестація → Налаштування (Vercel Blob + VERCEL_BLOB_READ_WRITE_TOKEN).",
        },
        { status: 409 },
      );
    }
    console.error("[attestation certificate doc]", e);
