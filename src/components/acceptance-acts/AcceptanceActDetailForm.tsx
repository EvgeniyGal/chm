    ...
    <FormProvider {...form}>
      ...
      >
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          ...
          <ReadOnlyField
            label="Дата акту"
            value={new Date(actDateIso).toLocaleDateString("uk-UA")}
          />
          <Field
        ...
        <label className="flex flex-col gap-1 text-sm">
          <span className="text-zinc-700">Місце складання</span>
          <textarea
            rows={2}
            disabled={!canEdit}
            className="min-h-10 w-full rounded-md border px-3 py-2 disabled:cursor-default disabled:bg-muted"
            {...register("signingLocation", { required: true })}
          />
        </label>
        ...
        <div className="grid grid-cols-1 gap-4 rounded-lg bg-muted/40 p-4 md:grid-cols-2">
          ...
          <Field label="Посада (називний)" disabled={!canEdit} {...register("signerPositionNom", { required: true })} />
          <Field label="Посада (родовий)" disabled={!canEdit} {...register("signerPositionGen", { required: true })} />
        </div>
        ...
        <div className="mt-2 flex flex-wrap items-center gap-3">
          ...
          <a
            ...
            aria-label="Завантажити акт (DOCX)"
...
function Field({
  ...
  return (
    <label className="flex flex-col gap-1 text-sm">
      ...
      <input {...props} className="h-10 rounded-md border px-3 disabled:cursor-default disabled:bg-muted" />
    </label>
  );
}
