...
export function SampleMaterialsTable({ rows }: { rows: SampleMaterialRow[] }) {
  ...
  async function confirmArchive() {
    ...
    try {
      ...
      await archiveSampleMaterialAction(fd);
      toast.success("Матеріал архівовано.");
  ...
  async function restoreRow(id: string) {
    ...
    try {
      const fd = new FormData();
      fd.set("id", id);
      await restoreSampleMaterialAction(fd);
      toast.success("Матеріал відновлено з архіву.");
      setEditingId(null);
      router.refresh();
    } catch (e) {
      toast.error(getServerActionErrorMessage(e));
  ...
  const columns = useMemo<ColumnDef<SampleMaterialRow>[]>(
    () => [
      {
              ...
              {r.isActive ? (
                <>
                  ...
                  <button
                    ...
                    onClick={() => setArchiveConfirm({ id: r.id, groupCode: r.groupCode, steelGrade: r.steelGrade })}
    ...
    return (
      <form
        ...
        action={async (formData) => {
          try {
            await updateSampleMaterialAction(formData);
      ...
      >
        ...
        <div className="text-xs font-medium text-foreground">Редагування матеріалу зразка</div>
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
          <label className="flex flex-col gap-1">
            <span className="text-xs text-muted-foreground">Група</span>
            <select
              name="groupCode"
              required
              defaultValue={row.groupCode}
          ...
          <label className="flex flex-col gap-1">
            <span className="text-xs text-muted-foreground">Марка сталі</span>
            <input
              name="steelGrade"
              required
              defaultValue={row.steelGrade}
              className="h-10 rounded-md border border-border bg-white px-3 text-sm dark:bg-background"
            />
          </label>
        </div>
  ...
  return (
    <>
      ...
      <ArchiveSettingsRowConfirmDialog
        ...
        title="Архівувати матеріал зразка?"
        message={
          archiveConfirm
            ? `Група ${archiveConfirm.groupCode}, марка «${archiveConfirm.steelGrade}» буде прихована з активного довідника.`
            : ""
        }
        archiving={archiving}
